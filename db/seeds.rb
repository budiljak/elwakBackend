...
#benutzers = Benutzer.create({login:'admin', passwort:'1234', nachname:"Admin", typ:VERWALTER})
...
while tag < DateTime.now do
  ...
  schichts = Schicht.create([
    {objekt: objekts[0], benutzer: benutzers[1], datum: tag, uhrzeit_beginn: tag + 2.hours, uhrzeit_ende: tag + 10.hours}, 
    {objekt: objekts[0], benutzer: benutzers[2], datum: tag, uhrzeit_beginn: tag + 10.hours, uhrzeit_ende: tag + 18.hours}, 
    {objekt: objekts[0], benutzer: benutzers[1], datum: tag, uhrzeit_beginn: tag + 18.hours, uhrzeit_ende: tag + 26.hours}
  ])
  schichts.each do |s|
    Rapport.create([
      {schicht: s, beschreibung: 'beschr1', ort:'ort1', uhrzeit: s.uhrzeit_beginn + 2.hours, massnahmen: 'massn1', position: 1}, 
      {schicht: s, beschreibung: 'beschr2', ort:'ort2', uhrzeit: s.uhrzeit_beginn + 4.hours, massnahmen: 'massn2', position: 2}, 
      {schicht: s, beschreibung: 'beschr3', ort:'ort3', uhrzeit: s.uhrzeit_beginn + 6.hours, massnahmen: 'massn3', position: 3}
    ])
    wb = WachbuchEintrag.create({schicht: s, besonderheiten: 'bes', schaeden: 'schä', schluessel_bemerkung: 'schlüBem'})
    Kontrollanruf.create({wachbuch_eintrag: wb, uhrzeit: s.uhrzeit_beginn + 2.hours, bemerkung: 'bem', objekt: 'obj'})
    Kontrollgang.create({wachbuch_eintrag: wb, uhrzeit: s.uhrzeit_beginn + 4.hours, bemerkung: 'bem'})
  end
  tag = tag + 1.days
end
