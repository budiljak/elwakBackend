...
while tag < DateTime.now do
  ...
  schichts.each do |s|
    Rapport.create([
      {schicht: s, beschreibung: 'beschr1', ort:'ort1', uhrzeit: (s.uhrzeit_beginn + 2.hours).strftime("%F"), massnahmen: 'massn1', position: 1}, 
      {schicht: s, beschreibung: 'beschr2', ort:'ort2', uhrzeit: (s.uhrzeit_beginn + 4.hours).strftime("%F"), massnahmen: 'massn2', position: 2}, 
      {schicht: s, beschreibung: 'beschr3', ort:'ort3', uhrzeit: (s.uhrzeit_beginn + 6.hours).strftime("%F"), massnahmen: 'massn3', position: 3}
    ])
